/*
    ...
    @problem: https://codeforces.com/edu/course/2/lesson/7/2/practice/contest/289391/problem/F
...
void Excalibur(){
    ...
    while (lim < m) {
        ...
        int idx=lim, cnt=0;
        rep(i, n) {P[i+1]=-1;R[i+1]=0;}
        mx=LLONG_MIN;
        mn=LLONG_MAX;
        while (idx < m) {
            auto ed = A[idx].second;
            if (uni(ed.first, ed.second)) {
                mn = min(mn, 1LL*A[idx].first);
                mx = max(mx, 1LL*A[idx].first);
                ++cnt;
            }
            ++idx;
        }
